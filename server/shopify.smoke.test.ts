...
const FINAL_CATALOG = [
  { title: "Xatirə - Limited Edition Vinyl", price: "40.00" },
  { title: "FATİHİ - Tote Bag", price: "15.00" },
  { title: "CavidanFatihi - Keyrings", price: "8.00" },
  { title: "FATİHİ - Cup", price: "10.00" },
  { title: "FATİHİ - Pullover Hoodie", price: "50.00" },
  { title: "CF - Hat", price: "15.00" },
] as const;
...
describe.skipIf(!configured)("shopify smoke (live)", () => {
  ...
  it(
    "returns exactly the approved six-product catalog with valid imagery",
    { timeout: 30_000 },
    async () => {
      const products = await listProducts({ first: 10 });

      expect(products).toHaveLength(FINAL_CATALOG.length);

      for (const expected of FINAL_CATALOG) {
        const product = products.find(item => item.title === expected.title);
        expect(product, `Missing approved product: ${expected.title}`).toBeTruthy();
        expect(Number.parseFloat(product?.priceRange.min.amount ?? "NaN")).toBe(Number.parseFloat(expected.price));
        expect(product?.priceRange.min.currencyCode).toBe("AZN");
        expect(product?.images[0]?.url, `Missing image: ${expected.title}`).toMatch(/^https:\/\//);
        expect(product?.images[0]?.url, `Failed image placeholder: ${expected.title}`).not.toMatch(/\.svg(?:\?|$)/i);
      }
    }
  );
});
