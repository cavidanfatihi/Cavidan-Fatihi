...
const CONTACT_API_URL = "https://cavidanfat-q9hobjvq.manus.space/api/contact";
...
export function BookingForm({ locale }: { locale: Locale }) {
  ...
  const submitInquiry = async (values: BookingValues) => {
    setStatus(null);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(submitInquiry)} className="grid gap-5">
        ...
        <Button type="submit" disabled={isSubmitting} className="mt-2 h-13 w-fit rounded-none bg-white px-7 text-[.72rem] font-bold uppercase tracking-[.14em] text-black hover:bg-zinc-200">{isSubmitting ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Send className="mr-2 size-4" />}{copy.send}</Button>
